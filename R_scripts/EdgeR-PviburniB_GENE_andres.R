...
genetotranscript<-read.delim("annotation/p.viburni.freeze.v0.braker.transcripts.to.genes.txt", header=FALSE)
genetotranscript <-genetotranscript[order(genetotranscript$V1),]

interpro<-read.delim("annotation/p.viburni.freeze.v0.braker.interproscan.tsv", header = FALSE) #GO terms with transcript id but no gene id
interpro <-interpro[order(interpro$V1),]

sampleinfo<-read.csv("/Users/agarcia/Documents/genomics/B_viburni_ross_lab/R_scripts/sampleinfoPviburniB.csv") #sample group info
...
rsem.counts <-read.delim("/Users/agarcia/Documents/genomics/B_viburni_ross_lab/R_scripts/RSEM_digi.counts.matrix",header=TRUE) #matrix generated from rsem
a <- rsem.counts
