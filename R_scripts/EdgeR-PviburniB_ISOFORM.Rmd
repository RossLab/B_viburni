---
title: "B viburni genome based -  ISOFORM LEVEL DIFFERENTIAL EXPRESSION ANALYSIS"
author: "Isabelle Vea"
date: "21/10/2020"
...
a<-read.delim("RSEM_digi_isoform.counts.matrix",header=TRUE) #matrix generated from rsem
head(a,2) 
...
proportion_0_count= 23194/2483
proportion_0_count
```
About 9% of samples have 0 transcript counts across all samples => these can be easily filtered out.
...
```{r}
pdf("rsem_isoform_mdplot-prenorm.pdf", width=10, height=10, pointsize=12)
...
```{r}
pdf("rsem_isoform_mdplot-postnorm.pdf", width=10, height=10, pointsize=12)
par(mfrow = c(1, 1)) # 2 rows, 2 columns
for (i in 1:ncol(x)) {
	plotMD(cpm(x, log=TRUE), column=i, xlab = "Average log-expression", ylab = "Expression log-ratio (this sample vs others)", main = colnames(x)[i])
	abline(h=0, col="red", lty=2, lwd=2)
...
```{r}
pdf("rsem_isoform_qualitycontrol.pdf", width=10, height=5, pointsize=12)
par(mfrow = c(1, 2)) 
...
pdf("rsem_isoform_genomebased_MDSplot-per-factor.pdf", width=10, height=10, pointsize=12)
par(mfrow=c(2,2))
...
Groupings look the same at the isoform level
...
pdf("rsem_isoform_genomebased_heatmapbysex.pdf", width=10, height=10, pointsize=12)
...
pdf("rsem_isoform_genomebased_heatmapbyB.pdf", width=10, height=10, pointsize=12)
...
```{r}
pdf("rsem_isoform_genomebased_meanvariancetrends.pdf", width=10, height=5, pointsize=12)
par(mfrow=c(1,2))
v1 <- voom(x,design1,plot = TRUE)
...
pdf("rsem_isoform_genomebased_vennDEcommon.pdf", width=10, height=10, pointsize=12)
par(mfrow=c(2,2))
...
maleB.noB_annot = merge(maleB.noB,annot, by.x='genes', by.y='tid',all.x=TRUE)
write.csv(maleB.noB_annot,"rsem_isoform_maleB.noB_annot.csv")

maleB.femaleB_df<-data.frame(maleB.femaleB)
maleB.femaleB_annot = merge(maleB.femaleB,annot, by.x='genes', by.y='tid',all.x=TRUE)
write.csv(maleB.femaleB_annot,"rsem_isoform_maleB.femaleB.csv")

maleB.femalenoB_df<-data.frame(maleB.femalenoB)
maleB.femalenoB_annot = merge(maleB.femalenoB,annot, by.x='genes', by.y='tid',all.x=TRUE)
write.csv(maleB.femalenoB_annot,"rsem_isoform_maleB.femalenoB.csv")

femaleB.femalenoB_df<-data.frame(femaleB.femalenoB)
femaleB.femalenoB_annot = merge(femaleB.femalenoB,annot, by.x='genes', by.y='tid',all.x=TRUE)
write.csv(femaleB.femalenoB_annot,"rsem_isoform_femaleB.femalenoB.csv")

malenoB.femalenoB_df<-data.frame(malenoB.femalenoB)
malenoB.femalenoB_annot = merge(malenoB.femalenoB,annot, by.x='genes', by.y='tid',all.x=TRUE)
write.csv(malenoB.femalenoB_annot,"rsem_isoform_malenoB.femalenoB.csv")
...
colnames(Bmale) = "transcript"


Bmale_annot = merge(Bmale,annot, by.x='transcript', by.y='tid',all.x=TRUE)

Bmale_annot2<-Bmale_annot[!is.na(Bmale_annot$V2), ]
write.csv(Bmale_annot2,"rsem_isoform_Bmale_annot2.csv")
```

#### MDplots for fit.cont1
```{r}
pdf("rsem_isoform_genomebased_MDplots.pdf", width=20, height=10, pointsize=12)
par(mfrow=c(2,3))
