...
setwd("/Users/isabelle/Dropbox/_Github_reps/B_viburni/")

genetotranscript<-read.delim("/Users/isabelle/Downloads//p.viburni.freeze.v0.braker.transcripts.to.genes.txt", header=FALSE)
genetotranscript <-genetotranscript[order(genetotranscript$V1),]

interpro<-read.delim("/Users/isabelle/Downloads/p.viburni.freeze.v0.braker.interproscan.tsv", header = FALSE) #GO terms with transcript id but no gene id
...
# A genes differentially expressed between B+ and B- lines regardless of sex


# define groups  B presence or absence only
group2=c("B","B","B","B","B","B","B","B","B","B","B","B","B","noB","noB","noB","noB","noB","noB","noB","noB","noB","noB","noB","noB","noB")

# design: Model Matrix by group defined above
design2 <- model.matrix(~0 + group2)
colnames(design2)
rownames(design2) = rownames(x$samples)

# removing heteroscedascity from count data: voom plots
v2 <- voom(x,design2,plot = TRUE)
v2
View(v2)
# limma lm fit
fit2 <- lmFit(v2)
...
summary(decideTests(fit.cont2))

# compare mean-variance trend
v2 <- voom(x,design2,plot = TRUE)
plotSA(fit.cont2, main="Final model: Mean-variance trend")

## Examine the number of DE genes

tfit <- treat(fit.cont2, lfc=1) 
dt <- decideTests(tfit)
summary(dt)

vennDiagram(dt[,1], circle.col=c("turquoise", "salmon","orange"),include=c("up","down"))

# these are all the genes that are over expressed and underexpressed, respectively, between B and non B lines
de.over.B <- which(dt[,1]==1)
de.under.B <- which(dt[,1]==-1)
length(de.over.B) #  genes overexpressed in B lines
length(de.under.B) #  genes underexpressed in B lines

# list of genes from Bmale
fit.cont2$genes[de.over.B,]
fit.cont2$genes[de.under.B,]

# export overexpressed genes in B+ vs B-
de.over.B.genes <- data.frame(fit.cont2$genes[de.over.B,])
colnames(de.over.B.genes) <- "genes"
#write.csv(de.over.B.genes,"output/diff_expr/over.B.vs.nonB.csv")
head(de.over.B.genes)

# export UNDERexpressed genes in B+ vs B-
de.under.B.genes <- data.frame(fit.cont2$genes[de.under.B,])
colnames(de.under.B.genes) <- "genes"
#write.csv(de.under.B.genes,"output/diff_expr/under.B.vs.nonB.csv")
head(de.under.B.genes)

# export list of genes 
B.noB <- topTreat(tfit, coef=1,number = summary(dt)[1]+summary(dt)[3])
write.csv(B.noB,"output/diff_expr/B.vs.nonB.de.treat.csv")
